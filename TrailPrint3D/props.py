...
class TP3D_PG_properties(bpy.types.PropertyGroup):
    ...
    el_wcMinFeatureArea: FloatProperty(name=_("Min Feature Area"), default=2.0, min=0.0, soft_max=100.0, description=_("Land-cover regions smaller than this area (map units²) are merged into whichever neighboring class actually surrounds them, instead of showing up as an isolated fleck. At a 100mm map size, 1 map unit ≈ 1mm on the print, so the default 2.0 ≈ a ~1.4×1.4mm patch — the same convention as Min Island Area above. Set to 0 to disable and use WorldCover's raw per-pixel classification.")) # type: ignore
