...
class TP3D_PG_properties(bpy.types.PropertyGroup):
    ...
    api: bpy.props.EnumProperty(
        ...
        items=[
            ...
            ("LOCAL_DEM", "Local DEM File", _("Sample elevation from a GeoTIFF DEM file you downloaded — no internet required"))
        ],
        default = "MAPTERHORN"
    )# type: ignore
    ...
    # ESA WorldCover per-category color toggles --Default
    ...
    el_bActive: BoolProperty(name= _("Include Buildings"), default=True, description = _("For Maps < 5Km Reccomended")) # type: ignore
    ...
    el_wcMinFeatureArea: FloatProperty(name=_("Min Feature Area"), default=0.4, min=0.0, soft_max=100.0, description=_("Land-cover regions smaller than this area (map units²) are merged into whichever neighboring class actually surrounds them, instead of showing up as an isolated fleck. At a 100mm map size, 1 map unit ≈ 1mm on the print. Set to 0 to disable and use WorldCover's raw per-pixel classification.")) # type: ignore
